...
Whenever possible, libuv will use those multi-threaded interfaces, but there are few tasks that requires using of internal libuv's thread pool.
...
`poll:` Event Loop Retrieve new I/O events, execute I/O related callbacks (almost all with the exception of close callbacks, the ones scheduled by `timers`, and `setImmediate()`).<br />
...
If Event loop in working inside some phase, it will execute callbacks until current queue is **empty** or the maximum number of callbacks are reached. And only after that event loop will move to the next phase.
...
#### poll queue
1. First when Event Loop inside poll queue it first checks if there are any **immediate callbacks**, if yes - then Event Loop proceeds to the `check phase`.
2. Poll queue is empty:
   ...
   - When the poll phase becomes idle and there is a callback that has been scheduled with using `setImmediate()`, the Event Loop will enter the `check phase` instead of waiting for new I/O events.
...
#### check
In this phase, callbacks scheduled using `setImmediate()` is executed - not the timer phase.
...
```javascript
/**
 * Order of this block depend on system workload, could be different for every execution
 */
...
To what queues does each callback belong to?
...
Starting with node `v11`, `nextTick queue` callbacks and `microtasks` will run between each individual `setTimeout` and `setImmediate` callbacks, even if the timers queue or the check queue is not empty. Check example bellow.
...
Promises have their own special treatment. **Microtask queue** run a lot more often that other queues event compared to the **nextTick queue**. Check example below:
...
<h3>Thank you for reading, I really hope that this post was useful for you.</h3>