...
authors: ['default']
draft: false
summary: Why can't we use hooks conditionally? How do hooks work? Where is data stored? What is React Fiber? We are gonna try to answer on all those questions together. 
---


## Prerequisites 
In this short post about react hooks we are going to start from **Conclusion** 🎉🎉🎉

### Conclusion
1. **What are hooks?**<br />
    React hooks allow usage of different React features inside functional components.
...
3. **Why can't we use hooks conditionally?**<br />
    ...
    If the order of the hook execution is different between re-renders - **nodes inside `FiberNode` structure lose a pointer and points to the totally different thing and whole structure simply can't work properly anymore.**
...
Fiber Tree consist of nodes called - `FiberNode`.
...
React will write that default values of the each individual `useState` hook and the order they're called **inside** `FiberNode`.<br />
...
export default App;
```
In this example we have two peaces of state with default value: `count="0"` and `name="John"`;
Now take a look how this sate is going to be represented inside `FiberNode`
![Hooks](/static/images/hooks/hooks_1.png)
As you can see order of the hooks execution strictly the same as its in our component above, 
and one useState has a pointer to another one `FiberNode -> memoizedState -> next ->`. You may already guess where I'm going with it. 

So what happens after re-render? React is going to execute app and to pass the values that were `kept in memory` inside the `FiberNode`. <br />
So the first state `const [count, setCount] = useState<number>(0);` is going to be reading - into the **first element** of the linked list 
and the second state `const [name, setName] = useState<string>("John");` is going to be reading - into the **second element** of the linked list (into the property `.next` of the first one). <br />
What if at the next render you add or remove a hook, in that case you change the order of the hooks execution and one or more of your hooks are going to point to the **irrelevant** data, to the data that is not related to that hook.
