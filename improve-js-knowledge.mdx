...
draft: false
summary: This post contains interesting TS/JS questions (and answers) that probably every developer asked yourself several times. Why this peace of code runs before that one? Why this expression is so weird etc.?
---
...
Most of the topics gonna be something like: _guess the output_ or _why it does'n work_.
...
3. Immediately invoked function expression **IIFE**:
    <Callout size="xs">
        <p className="p-0 m-0 tracking-tight leading-tight">All code after `await` statement considered as `microtask`, until that code considered as **synchronous** code.</p>
    </Callout>
    ...
    // 1, 2, 3
    ```
4. Since call stack is empty now, EventLoop can process queues: microtask and after macrotask. 
And first item in microtask queue is resolved promise: `then(() => console.log(1)`.
5. Next `queueMicrotask(() => console.log(2));` - based on name it's obvious from what queue this callback came to call stack.
...
2. First thing in its body is: `console.log(4);` pushed to the callstack, executed, popped out.
...
8. Body of `asyncFunction` executed, pushed to the its queues - we are still **awaiting** and `asyncFunction` itself pushed to the **microtask queue** and EventLoop will execute, rest of the code.
9. `res();` pushed to the stack, `await new Promise` popped out from stack.
...
{/* <Video className="m-0 h-72 w-full mx-auto" src="/static/images/improve-js-knowledge/event_loop_hard_question.mp4" /> */}
<Divider />
</details>
...
Because `new Promise` defined on line 4, was never resolved - it's remained in `pending` state.</p>
<Callout size="xs">
...
```
<p className="m-0">When one function defined inside another - inner function has the access to the outer function's scope. 
JavaScript **closure** it is the combination of a function and references to its the lexical environment. 
...
to fix bug new object must be returned. But I think that is great example of the **closure in JavaScript**.</p>
<p className="m-0 mb-1">When we are comparing object - we are compering just `reference in memory`.</p>
<Divider />
</details>
...
- a. Hoisting is a process of moving function and variables to the top of the file
- b. Variables declared with the `let` and `const` are hoisted
- c. Variables declared with the `var` keyword are uninitialized
- d. Hoisting ocurred during execution phase
- e. `import` declaration are hoisted
...
<p className="m-0 mt-2">When JS engine starts to work with any `.js` files it makes an environment called the **Execution Context** and it has two phases:</p>
1. Creation phase
2. Execution phase
...
- Stores the variables with the `var` keyword with default value `undefined` and function declarations name with the function reference.
...
and only variables with `var` keyword will have initial value as `undefined`, during the **execution phase**, all variables gonna be assigned to corresponding values.</p>
...
resetCounter();
```
<p className="m-0">As you may guess above code gonna throw an error `Uncaught ReferenceError: Cannot access 'counter' before initialization`.</p>
<p className="m-0">All JS code runs line by line during execution phase and when we are trying to log `console.log(counter);` 
JS engine **throws** an error - that counter isn't initialized - this means that `counter` variable is so called _hoisted_ - 
it already exists in our **Function lexical environment**, otherwise error would be totally different. Check image bellow:</p>
...
<Image
    ...
    height={204}
/>
<p className="m-0 mt-4">So shortly - **Hoisting** it's a process of setting default value to the corresponding declaration during creation phase.</p>
...
**Global context** - when `this` is used in global context the value of `this` gonna be global object.<br />
...
**Arrow function** -  when `this` is used in arrow function the value of `this` gonna be determined by the lexical environment in which an the arrow function is defined.<br />
**Classes** - when `this` is used in classes the value of `this` gonna be instance of that class.<br />
**Strict mode** - when `this` is pointing to global object in strict mode the value of `this` gonna be `undefined`.<br />
...
- `thatObj.printContext_1` - is just return result of `printContext` function, so nothing related to `thatObj`.
...
- `thatObj_2.bam` - we called `foo` method with `this` of `thatObj_2`. In case of object methods it doesn't matter where object method is defined - it's only matter how it's **invoked**.
...
- `printContext()` and `printContext_1()` - when those methods where destructured, the value of `this` was pointed to the context in which these methods were called, 
in this case global object.
<Divider />
</details>

<p className="my-3">Is it possible to iterate through object using `Array.forEach` method?</p>
