...
And libuv allows Node.js to perform asynchronous operations that uses native OS interfaces, that are _most of the time_ asynchronous by default. 
...
When you create new Worker using `worker_threads` module Node.js fires up a new JavaScript VM, initializes a new global context, sets up a new heap, starts up a new garbage collector, allocates memory, etc...
...
so you can fire up one or more WorkerThreads to handle CPU-intensive work and keep the main thead Event Loop free and responsive to incoming events/networking and so on.

### What is the difference between cluster and WorkerThreads in Node.js?
`cluster` module as allowing you to run multiple instances of Node.js in **separate processes**, while the `worker_thread` module allows you to run multiple instances of Node.js in the same process.
...
### CPU intensive task
This example shows us how to divide big job (in this case count to the `1e10` number) into smaller parallel chunk of work. 
The most simple as strait forward example is to count to some big number. We are going to use Node.js `WorkerThreads` module 
...
What data type to use?
    ...
    since WorkerThreads are fatty ones we wanna use as less memory as possible,
    but not in our case, since we are going to manipulate with a big numbers 
...
// helper func to sums up number in array
...
And that is it, just import `example` in you `main.go` and run using `go run main.go` command.
...
```
As may expect for CPU bound tasks - `Go` may be considered as better solution `Node.js`.
...
Local variables are much much more efficient. As may see using local variable job took: `723.72ms`, but with the global ones same job took: `3631.19ms`, which is **5x** slower.<br />
But why?
